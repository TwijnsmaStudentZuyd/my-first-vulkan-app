...
private:
	...
	// use srgb for the best colorspace look
	...
	// use mailbox if supported otherwise use fifo
	...
	void createSwapChain() {
		...
		// min and max number of images in the swapChain, 0 represents unlimited images
		uint32_t imageCount = swapChainSupport.capabilities.minImageCount + 1;
		if (swapChainSupport.capabilities.maxImageCount > 0 && imageCount > swapChainSupport.capabilities.maxImageCount) {
			imageCount = swapChainSupport.capabilities.maxImageCount;
		}

		// supply creation info
		...
		createInfo.imageArrayLayers = 1; // this is always 1 outside of stereoscopic 3d
		createInfo.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;

		QueueFamilyIndices indices = findQueueFamilies(physicalDevice);
		uint32_t queueFamilyIndices[] = { indices.graphicsFamily.value(), indices.presentFamily.value() };

		// this is neccesary because we're using multiple queues
		...
		// supply current to not transform
		createInfo.preTransform = swapChainSupport.capabilities.currentTransform;

		// don't try to blend alpha with other windows
